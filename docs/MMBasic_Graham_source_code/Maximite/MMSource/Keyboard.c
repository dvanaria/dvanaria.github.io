...
void initKeyboard(void) {
    ...
    PS2State = PS2START;
    KeyboardTimer = 0;                                              // mSec used to timeout on a bit stream error  KBErr
...
void __ISR( _CHANGE_NOTICE_VECTOR, ipl3) CNInterrupt(void) {
    ...
    // flag an error if the keyboard has not been used for 10 seconds and any state key is down
    if(KeyboardTimer <= -KB_STATE_TIMEOUT && (LShift || RShift || Ctrl || AltGrDown || KeyUpCode || KeyE0)) PS2State = PS2ERROR;
    
    // delay for 20 clock cycles (about 0.5uS)
    dly = ReadCoreTimer() + 20;
    while(ReadCoreTimer() < dly);
        ...
        switch(PS2State){
            ...
            case PS2START:   
                if(!d) {                							// PS2DAT == 0
                    ...
                    Code = 0;
                    PS2State = PS2BIT;
                 	KeyboardTimer = KB_BIT_TIMEOUT;                 // used to timeout on a bit stream error  KBErr
                }
                break;
            ...
            case PS2STOP:    
                ...
                KeyboardTimer = -1;                                      // used to timeout on a bit stream error  KBErr
                PS2State = PS2START;
                break;
	    } 
	}
