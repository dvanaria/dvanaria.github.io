...
volatile int KeyboardTimer = 0;   // KBErr
...
void __ISR( _TIMER_4_VECTOR, ipl1) T4Interrupt(void) {
	...
	// used to timeout on error when receiving bits from the keyboard  KBErr
	if(KeyboardTimer > -KB_STATE_TIMEOUT) {
    	KeyboardTimer--;
    	if(KeyboardTimer == 0) PS2State = PS2ERROR;
    }   	
