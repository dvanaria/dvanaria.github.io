...
void cmd_memory(void) {
    ...
    vim = 0;
    #if defined(COLOUR)
        if((unsigned int)VideoBufRed > (unsigned int)PMemory) vim += (VBuf * HBuf) / 8;
        if(VideoBufGrn != VideoBufRed && (unsigned int)VideoBufGrn > (unsigned int)PMemory) vim += (VBuf * HBuf) / 8;
        if(VideoBufBlu != VideoBufRed && VideoBufBlu != VideoBufGrn && (unsigned int)VideoBufBlu > (unsigned int)PMemory) vim += (VBuf * HBuf) / 8;
	#endif
	...
	MMPrintString(inpbuf);
        ...
        dp("\r\n     Colour Mode = %d", ModeC);
	#endif
	dp("\r\nStatic Variables = A0000000   Size = %dKB", ((unsigned int)&_heap - 0xA0000000)/1024); 
	dp("        C32 Heap = %p   Size = %d bytes", (unsigned char *)&_heap, (unsigned int)&_splim - (unsigned int)&_heap); 
    #if defined(COLOUR)
	    if(VideoBufRed && VideoBufRed <= (int *)PMemory) dp("  Red Video Buff = %p   Size = %dKB", VideoBufRed, ((VBuf * HBuf) / 8)/1024); 
	    if(VideoBufGrn && VideoBufGrn <= (int *)PMemory) dp("Green Video Buff = %p   Size = %dKB", VideoBufGrn, ((VBuf * HBuf) / 8)/1024); 
	    if(VideoBufBlu && VideoBufBlu <= (int *)PMemory) dp(" Blue Video Buff = %p   Size = %dKB", VideoBufBlu, ((VBuf * HBuf) / 8)/1024); 
	#else
	    dp("    Video Buffer = %p   Size = %dKB", VideoBuf, ((unsigned int)PMemory - (unsigned int)VideoBuf)/1024); 
	#endif
	...
	heapstats("                  ");
    #if defined(COLOUR)
	    if(VideoBufRed >  (int *)PMemory) dp("  Red Video Buff = %p   Size = %dKB", VideoBufRed, ((VBuf * HBuf) / 8)/1024); 
	    if(VideoBufGrn >  (int *)PMemory) dp("Green Video Buff = %p   Size = %dKB", VideoBufGrn, ((VBuf * HBuf) / 8)/1024); 
	    if(VideoBufBlu >  (int *)PMemory) dp(" Blue Video Buff = %p   Size = %dKB", VideoBufBlu, ((VBuf * HBuf) / 8)/1024); 
	#endif
	dp("     PIC32 Stack = %p   Size = %dKB  top = %p", (unsigned char *)&_stack - (unsigned int)&_min_stack_size - PAGESIZE, (unsigned int)&_min_stack_size/1024, (unsigned char *)&_stack); 
#endif
...
void m_alloc(int type, int size) {
    ...
    switch(type) {
        case M_VIDEO:   // this allocates memory for the video buffer
                        ...
                        #if defined(COLOUR)
                            VideoBufRed = (int *)(MRoundUp((unsigned int)&_splim));
                        #else
                            VideoBuf = (int *)(MRoundUp((unsigned int)&_splim));
                        #endif
                        VideoBufSize = MRoundUp(size);
                        break;
...
void InitHeap(void) {
    ...
    #if defined(COLOUR)
        CLine = NULL;
        // put back the allocations for any video buffers allocated from the heap
        if((unsigned int)VideoBufGrn >  (unsigned int)PMemory) {
            i = ((((VBuf * HBuf) / 8) + PAGESIZE - 1)/PAGESIZE) - 1;
            if(i < 0) return;  // sanity check
            MBitsSet(VideoBufGrn + (i * (PAGESIZE/4)), PUSED | PLAST);
            for(i--; i >= 0; i--) MBitsSet(VideoBufGrn + (i * (PAGESIZE/4)), PUSED);
        }
        if((unsigned int)VideoBufBlu >  (unsigned int)PMemory) {
            i = ((((VBuf * HBuf) / 8) + PAGESIZE - 1)/PAGESIZE) - 1;
            if(i < 0) return;  // sanity check
            MBitsSet(VideoBufBlu + (i * (PAGESIZE/4)), PUSED | PLAST);
            for(i--; i >= 0; i--) MBitsSet(VideoBufBlu + (i * (PAGESIZE/4)), PUSED);
        }
    #endif
}    
