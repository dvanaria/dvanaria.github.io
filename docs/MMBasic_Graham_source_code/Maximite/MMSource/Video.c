...
#include "Hardware_Includes.h"
	...
	// Parameters for the TFT Maximite for use with 4.3" PSP TFT, Sharp LQ043 by Carsten Meyer, cm@ct.de
	// Graphics is 480x272 pixels.  This gives us 80 chars per line and 22 lines per screen
	#define VGA_VRES	 	272										// 272 pixels Vert graphics resolution (pixels)
	#define VGA_HRES	 	480										// 480 pixels Horiz graphics resolution (pixels)
...
const unsigned int FontOption = 0xffffffff;					        // use to hold the default font setting in flash
...
void SetupSPI(int SPIchnl, int SPIint, int SPIpixt, void *SPIinput, int DMAchnl, int DMAsize, int *VBuffer) {
    ...
    // due to a bug in the PIC32 SPI (Microchip Errata #13) one or more nops may or may not be required to get the colours into proper
    // registration on the VGA monitor.  Try compiling with one or more of the following lines commented out or not commented
    // (ie, 1 nop, 2 nops, 3 nops or 4 nops)- one of the combinations should ensure that the colours are registered properly
    while(ReadCoreTimer() & 0b1111) nop;                            // ensure that each colour is started on the same clock phase
    nop;
    nop;
    nop;
    //nop;
...
void initVideo( void) {
	...
	// first check if video output is disabled, if so we can skip nearly everything
    if(GetFlashOption(&VideoOption) == CONFIG_OFF) {
        ...
        #endif
	    VRes = VGA_VRES; HRes = VGA_HRES; vga = VBuf = HBuf = 0;
	    return;
	}

#if defined(TFT_MAXIMITE)
    // if we are on the TFT Maximite then VGA is always on but we need to find the type of display used
    vga = true;
#else
	// on the ordinary Colour Maximite we have to check if we need to be in the VGA or composite mode
    ...
    vga = (P_VGA_COMP == P_VGA_SELECT) || (GetFlashOption(&PalVgaOption) == CONFIG_DISABLED);	// vga will be true if the jumper is NOT there or composite is disabled
#endif
    ...
    if(vga) {
	    ...
	    VBuf = VRes = VGA_VRES;                                     // VBus is the vert buffer size and VRes is that actual number of lines to use
...
//    screenWidth = HRes / (fontWidth * fontScale);
    ...
    if(vga) {
	    ...
	    SpiChnOpen(P_RED_SPI, SPICON_ON | SPICON_MSTEN | SPICON_MODE32 | SPICON_FRMEN | SPICON_FRMSYNC | SPICON_FRMPOL, C_PIX_T);
        DmaChnOpen(1, 1, DMA_OPEN_DEFAULT);    	                            // setup DMA 1 is the blank padding at the start of a scan line
        DmaChnSetEventControl(1, DMA_EV_START_IRQ_EN | DMA_EV_START_IRQ(P_RED_SPI_INTERRUPT));
	    DmaChnSetTxfer(1, (void*)zero, (void *)&P_RED_SPI_INPUT, C_BLANKPAD, 4, 4);
...
void __ISR(_TIMER_3_VECTOR, ipl7) T3Interrupt(void) {
    static unsigned char AlternateField = 0;                        // used to count odd/even fielda in composite video
    #if defined(COLOUR)
	    static int *VPtrRed;
	    static int *VPtrGrn;
	    static int *VPtrBlu;
    ...
    switch ( VState) {    											// vertical state machine
        ...
        case SV_POSTEQ: // 2
            ...
            #if !defined(TFT_MAXIMITE)
                P_VERT_SET_HI;							            // end of the vertical sync pulse for vga when not using the LCD display
            #endif
            break;
        ...
        case SV_LINE:   // 3
            #if defined(COLOUR)
                #if defined(TFT_MAXIMITE)
            	    P_VERT_SET_HI;					                // end of the vertical sync pulse for vga when using the LCD display
                #endif
                if(VPtrRed) {
                    #if !defined(TFT_MAXIMITE)
...
void ScrollUp(int edit) {
    ...
    if(!HBuf) return;                                                                  // video is turned off and there is no memory allocated

   	if(edit)
        #if defined(TFT_MAXIMITE)
   	        amount = (HBuf/32) * (VBuf - (fontHeight * fontScale) * 3 - 8);             // for the editor - reserve the bottom two lines
   	    #else
   	        amount = (HBuf/32) * (VBuf - (fontHeight * fontScale) * 3);                 // for the editor - reserve the bottom two lines
   	    #endif
   	else
   	    amount = (HBuf/32) * (VBuf - (fontHeight * fontScale));                         // for DrawChar() - scroll everything

    #if defined(COLOUR)
        if(VideoBufRed) {
    		pd = VideoBufRed;
           	ps = pd + (HBuf/32) * (fontHeight * fontScale);
           	for(i=0; i < amount; i++) *pd++ = *ps++;	                                // scroll up
...
void SetMode(int mc, int mp) {
	...
	switch(mc) {
        ...
        // two bit colour mode.  made more complicated by the palette selection
            if(tp2 == NULL) {
                VideoBufBlu = VideoBufGrn = VideoBufRed = tp1; ModeC = 1; ModeP = 7;    // this is a fail safe, in case the memory allocation fails
                tp2 = getmemory((VBuf * HBuf) / 8);
            }
            if(tp3) {
                ORbitmaps(tp2, tp3);
                FreeHeap(tp3);
        ...
        case 3:
            // 3 bit colour mode
            if(ModeC == 2) {
	            tx1 = getmemory((VBuf * HBuf) / 8);                         // this is a fail safe, in case the memory allocation fails
                VideoBufRed = tp1;
                VideoBufGrn = tp2;
                ORbitmaps(tp1, tp2);
                ORbitmaps(tp2, tp1);
            ...
            } else if(ModeC == 1 || ModeC == 4) {
                ModeC = 1;  ModeP = 7;                                      // this is a fail safe, in case the memory allocation fails
	            tx1 = getmemory((VBuf * HBuf) / 8); tx2 = getmemory((VBuf * HBuf) / 8);
                VideoBufGrn = tx1; VideoBufBlu = tx2;                       // OK, the test passed so now we can now set the video pointers
                ORbitmaps(VideoBufGrn, tp1); ORbitmaps(VideoBufBlu, tp1);   // and set the colour of any remaining image to white
            }
    	    break;
    	case 4:
