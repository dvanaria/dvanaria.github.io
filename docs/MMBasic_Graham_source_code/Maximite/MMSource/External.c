...
void cmd_port(void) {
    ...
    for(i = 0; i < argc; i += 4) {
        ...
        while(nbr) {
            if((PinDef[pin].mode & UNUSED) || !(ExtCurrentConfig[pin] == EXT_DIG_OUT || ExtCurrentConfig[pin] == EXT_OC_OUT)) error("Invalid output pin");
            ExtSet(pin, value & 1);
            value >>= 1;
            nbr--;
            pin++;
...
void fun_port(void) {
    ...
    for(i = argc - 3; i >= 0; i -= 4) {
        ...
        while(nbr) {
            if((PinDef[pin].mode & UNUSED) || !(ExtCurrentConfig[pin] == EXT_DIG_IN || ExtCurrentConfig[pin] == EXT_INT_HI || ExtCurrentConfig[pin] == EXT_INT_LO || ExtCurrentConfig[pin] == EXT_INT_BOTH)) error("Invalid input pin");
            value <<= 1;
            value |= PinRead(pin);
            nbr--;
            pin--;
